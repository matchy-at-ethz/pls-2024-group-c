import numpy as np
from DynamicControlModule import PID
...
class DataStruct():
    ...
    def duplicateAllValues(self):
        ...
        """
        self.timestamps = np.hstack((self.timestamps, self.timestamps[:,-1].reshape(-1,1).copy()))
        self.concentration_tf_mrna = np.hstack((self.concentration_tf_mrna, self.concentration_tf_mrna[:, -1].reshape(-1, 1).copy()))
        self.concentration_tf = np.hstack((self.concentration_tf, self.concentration_tf[:,-1].reshape(-1, 1).copy()))
        self.concentration_t_rna = np.hstack((self.concentration_t_rna, self.concentration_t_rna[:,-1].reshape(-1, 1).copy()))
        self.concentration_t = np.hstack((self.concentration_t, self.concentration_t[:,-1].reshape(-1, 1).copy()))
        self.concentration_mi_rna =np.hstack((self.concentration_mi_rna, self.concentration_mi_rna[:,-1].reshape(-1, 1).copy()))
        self.concentration_complex = np.hstack((self.concentration_complex, self.concentration_complex[:,-1].reshape(-1, 1).copy()))
    ...
    def determineEvent(self, r):
        ...
        boundaries = [(self.tf_rna_increase, 0), (self.tf_rna_part, 0),
                      (self.tf_increase, 1), (self.tf_part,1),
                      (self.t_rna_increase, 2), (self.t_rna_part, 2),
                      (self.t_increase, 3), (self.t_part, 3),
                      (self.mi_rna_increase, 4), (self.mi_rna_part, 4),
                      (self.complex_increase, 5), (self.complex_part, 5)]
        ...
        for boundary, name in boundaries:
            ...
            if not increase:
                upper_bound -= boundaries[count-1][0]*r #subtract the probabillity for increase as its contained in the partition
            
            if rand_float < upper_bound: #no need to explicitly check for the lower bound, as it has already been tested
                
                return (name , increase, False) #I discarded the errorhandling later, now i just try again. it may be that 
    ...
    def evaluatePartitionFunction(self, trajectory):
        ...
        self.tf_increase = self.mu_t*self.concentration_tf_mrna[trajectory, step] #increase reaction
        self.tf_part = self.mu_q*q #partition function for this

        self.tf_rna_increase = self.alpha_t
        self.tf_rna_part = self.alpha_t + self.concentration_tf_mrna[trajectory, step]*self.mu_t

        self.mi_rna_increase = self.alpha_s*q/(q + self.k_s) + self.mu_c*self.concentration_complex[trajectory, step]
        self.mi_rna_part = self.mi_rna_increase + self.mu_s*self.concentration_mi_rna[trajectory, step]

        self.t_rna_increase = self.alpha_r*q/(q + self.k_r)
        self.t_rna_part = self.t_rna_increase + self.mu_r*self.concentration_t_rna[trajectory, step]

        self.t_increase = self.pi_h*self.concentration_t_rna[trajectory, step]
        self.t_part = self.t_increase + self.mu_h*self.concentration_t[trajectory, step]

        self.complex_increase = self.beta*self.concentration_t_rna[trajectory, step]*self.concentration_mi_rna[trajectory, step]
        self.complex_part =  self.complex_increase + self.mu_c*self.concentration_complex[trajectory, step]

        inv_r = self.tf_part + self.tf_rna_part + self.mi_rna_part + self.t_rna_part + self.t_part + self.complex_part
