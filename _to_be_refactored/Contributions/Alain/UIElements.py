...
class SplitContainer:
    ...
    def __init__(
        ...
        if sizemode == "xExpand":
            self.panel1_height = (
                ...
                - 1.5 * (self.padding["l"] + self.padding["b"])
            ) * fraction
    ...
    def update_in_fill(self):
        ...
        if self.orientation == "h":
            ...
            panel1_width = self.size["x"] - (
                self.padding["l"] + self.padding["r"]
            )  # account for split
            panel1_height = (self.size["y"] * self.fraction) - 1.5 * (
                self.padding["t"] + self.padding["b"]
            )
            ...
            panel2_posy = (
                panel1_posy
                + panel1_height
                + (self.padding["t"] + self.padding["b"]) / 2
            )
            panel2_width = panel1_width
            panel2_height = (self.size["y"] - panel1_height) - (
                self.padding["t"] * 1.5 + self.padding["b"]
            )
        ...
        if self.orientation == "v":
            ...
            panel1_width = (self.size["x"] * self.fraction) - 1.5 * (
                self.padding["l"] + self.padding["r"]
            )
            ...
            panel2_posx = (
                panel1_posx
                + panel1_width
                + 0.5 * (self.padding["l"] + self.padding["r"])
            )
            panel2_posy = panel1_posy
            panel2_height = panel1_height
            panel2_width = (self.size["x"] - panel1_width) - 1.5 * (
                self.padding["l"] + self.padding["b"]
            )

        self.panel1.place_configure(
            height=panel1_height, width=panel1_width, x=panel1_posx, y=panel1_posy
        )
        self.panel2.place_configure(
            height=panel2_height, width=panel2_width, x=panel2_posx, y=panel2_posy
        )
    ...
    def position_on_split(self, x, y, margin):
        ...
        if self.orientation == "h":
            ...
            lower_bound = (self.size["y"] * self.fraction) - margin * (
                self.padding["t"] + self.padding["b"]
            )
        ...
        if self.orientation == "v":
            ...
            lower_bound = (self.size["x"] * self.fraction) - margin * (
                self.padding["l"] + self.padding["r"]
            )
...
class MenuBand:
    ...
    def setup_windows(self):
        ...
        self.frame.place_configure(
            height=self.size["y"], width=self.size["x"], x=0, y=0
        )
